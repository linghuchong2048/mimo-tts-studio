...
interface TTSProvider {
  voiceClone(params: VoiceCloneParams, apiKey: string, endpoint: string): Promise<TTSResult>;
  voiceDesign(params: VoiceDesignParams, apiKey: string, endpoint: string): Promise<TTSResult>;
}
...
interface LLMProvider {
  chat(params: LLMChatParams, apiKey: string, endpoint: string): Promise<string>;
}

function getDefaultTTSSettings(): TTSSettings {
  return {
    ttsBackend: "mimo-cloud",
    localTtsEndpoint: "http://localhost:8000"
  };
}
...
async function readTTSSettings(): Promise<TTSSettings> {
  const apiSettings = await readApiSettings();
  return {
    ttsBackend: apiSettings.ttsBackend || "mimo-cloud",
    localTtsEndpoint: "http://localhost:8000"
  };
}
...
async function readLLMSettings(): Promise<LLMSettings> {
  const apiSettings = await readApiSettings();
  return {
    llmApiKey: apiSettings.llmApiKey || "",
    llmEndpoint: apiSettings.llmEndpoint || "",
    llmModel: apiSettings.llmModel || "deepseek-chat"
  };
}

// ====== MiMo Cloud TTS Provider ======

const mimoCloudProvider: TTSProvider = {
  async voiceClone(params: VoiceCloneParams, apiKey: string, endpoint: string): Promise<TTSResult> {
    const payload: MimoPayload = {
      ...
      audio: {
        format: "wav",
        voice: `data:${params.audioMimeType};base64,${params.audioBase64}`
      }
    };
    ...
    if (!response.ok) {
      throw Object.assign(
        new Error(`MiMo voice clone failed: HTTP ${response.status}`),
        { status: response.status, elapsedMs, details: parsed ?? responseText, payload }
      );
    }
    ...
    if (!audioData) {
      throw Object.assign(
        new Error("MiMo API response did not include audio data."),
        { elapsedMs, details: parsed, payload }
      );
    }

    return { audioBase64: audioData, elapsedMs };
  },

  async voiceDesign(params: VoiceDesignParams, apiKey: string, endpoint: string): Promise<TTSResult> {
    const payload: MimoVoiceDesignPayload = {
      model: "mimo-v2.5-tts-voicedesign",
      messages: [
        { role: "user", content: params.voiceDescription },
        { role: "assistant", content: params.text }
      ],
      audio: { format: "wav" }
    };
    ...
    if (!response.ok) {
      throw Object.assign(
        new Error(`MiMo voice design failed: HTTP ${response.status}`),
        { status: response.status, elapsedMs, details: parsed ?? responseText, payload }
      );
    }
...
// ====== 本地 OmniVoice TTS Provider ======

const LOCAL_TTS_URL = "http://localhost:8000";

async function localVoiceDesign(params: VoiceDesignParams): Promise<TTSResult> {
  const body = { voiceDescription: params.voiceDescription, text: params.text, format: params.format };
  const startedAt = Date.now();
  const response = await fetch(`${LOCAL_TTS_URL}/api/tts/voicedesign`, {
    method: "POST",
  ...
  if (!response.ok) {
    ...
    throw Object.assign(new Error(`Local TTS voice design failed: HTTP ${response.status}`), { status: response.status, elapsedMs, details: errorText });
  }

  const parsed = parseJson(await response.text());
  const audioData = extractAudioData(parsed);
  if (audioData) return { audioBase64: audioData, elapsedMs };

  const data = parsed as { audioDataUrl?: string } | null;
  const match = data?.audioDataUrl?.match(/^data:[^;]+;base64,(.+)$/);
  if (match) return { audioBase64: match[1], elapsedMs };

  throw Object.assign(new Error("Local TTS response did not include audio data."), { elapsedMs });
}

async function localVoiceClone(params: VoiceCloneParams): Promise<TTSResult> {
  const body = {
    audioDataUrl: `data:${params.audioMimeType};base64,${params.audioBase64}`,
    text: params.text,
    instruction: params.instruction,
    format: params.format
  };
  const startedAt = Date.now();
  const response = await fetch(`${LOCAL_TTS_URL}/api/tts/voiceclone`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body)
  });
  ...
  if (!response.ok) {
    const errorText = await response.text().catch(() => "");
    throw Object.assign(new Error(`Local TTS voice clone failed: HTTP ${response.status}`), { status: response.status, elapsedMs, details: errorText });
  }
...
const localTTSProvider: TTSProvider = {
  voiceClone: (params) => localVoiceClone(params),
  voiceDesign: (params) => localVoiceDesign(params)
};

async function getTTSProvider(): Promise<{ provider: TTSProvider; settings: TTSSettings }> {
  const settings = await readTTSSettings();
  if (settings.ttsBackend === "local") {
    return { provider: localTTSProvider, settings };
  }
  return { provider: mimoCloudProvider, settings };
}

// ====== LLM Provider ======

async function callOpenAICompatibleLLM(params: LLMChatParams, apiKey: string, endpoint: string): Promise<string> {
  ...
  console.log(`[LLM] Calling ${url} with model ${params.model}`);
  ...
  if (!response.ok) {
    ...
    console.error(`[LLM] Error response: ${preview}`);
    throw Object.assign(new Error(`LLM request failed: HTTP ${response.status} - ${preview}`), { status: response.status, details: responseText });
  }
...
// 获取 LLM 配置。如果用户配置了自定义 LLM，使用它；否则用 MiMo API。
async function getLLMConfig(req: { headers: Record<string, string | string[] | undefined> }): Promise<{
  apiKey: string;
  endpoint: string;
  model: string;
}> {
  const llmSettings = await readLLMSettings();
  if (llmSettings.llmEndpoint && llmSettings.llmApiKey) {
    return {
  ...
  if (!apiKey) {
    throw Object.assign(new Error("API Key 未配置。"), { status: 500 });
  }
  return {
    apiKey,
    endpoint: apiEndpoint,
    model: "" // MiMo 调用会自行指定 model
  };
}

// 统一的 LLM 调用入口：如果配置了自定义 LLM（DeepSeek 等），走自定义；否则走 MiMo API。
async function callLLM(
  req: { headers: Record<string, string | string[] | undefined> },
  payload: MimoChatPayload
): Promise<string> {
  const { apiKey, apiEndpoint } = getApiConfig(req);
  return callLLMWithFallback(payload, apiKey ?? "", apiEndpoint);
}

// 独立版本，供辅助函数使用（不需要 req 对象）
async function callLLMWithFallback(
  ...
  if (!fallbackApiKey) {
    throw Object.assign(new Error("MIMO_API_KEY is not configured."), { status: 500 });
  }

  const response = await fetch(fallbackEndpoint, {
    method: "POST",
    headers: { "api-key": fallbackApiKey, "Content-Type": "application/json" },
    body: JSON.stringify(payload)
  });

  const text = await response.text();
  if (!response.ok) {
    throw Object.assign(new Error(`MiMo API error: HTTP ${response.status}`), { status: response.status, details: text });
  }

  const content = extractMessageContent(parseJson(text));
...
app.get("/api/settings", async (_req, res, next) => {
  try {
    ...
    const tts = await readTTSSettings();
    const llm = await readLLMSettings();
    res.json({
...
app.put("/api/settings", async (req: Request<unknown, unknown, ApiSettings>, res, next) => {
  try {
    ...
    const ttsBackend = (req.body.ttsBackend === "mimo-cloud" || req.body.ttsBackend === "local") ? req.body.ttsBackend : undefined;
    const llmApiKey = typeof req.body.llmApiKey === "string" ? req.body.llmApiKey.trim() : undefined;
    const llmEndpoint = typeof req.body.llmEndpoint === "string" ? req.body.llmEndpoint.trim() : undefined;
    const llmModel = typeof req.body.llmModel === "string" ? req.body.llmModel.trim() : undefined;

    if (!apiKey) {
      return res.status(400).json({ error: "API Key is required." });
    }

    const settings: ApiSettings = { apiKey, apiEndpoint: apiEndpoint || mimoEndpoint };
    if (ttsBackend) settings.ttsBackend = ttsBackend;
    if (llmApiKey !== undefined) settings.llmApiKey = llmApiKey || undefined;
    if (llmEndpoint !== undefined) settings.llmEndpoint = llmEndpoint || undefined;
    if (llmModel !== undefined) settings.llmModel = llmModel || undefined;
...
app.post("/api/voice-style/optimize", async (req: Request<unknown, unknown, VoiceStyleOptimizePayload>, res, next) => {
  ...
  try {
    ...
    const optimizedText = await callLLM(req, {
      model: "mimo-v2-flash",
      messages: [
        {
          role: "system",
    ...
    res.json({ optimizedText: optimizedText.trim(), elapsedMs });
  } catch (error) {
    next(error);
  }
});
  ...
  try {
    ...
    if (!plan) {
      return res.status(502).json({
        error: "Smart workspace response was not valid JSON.",
        elapsedMs,
        details: content
      });
    }
    ...
    if (!segments) {
      return res.status(502).json({
        error: "Smart workspace segment count did not match the script paragraphs.",
...
app.post("/api/audiobook/:id/characters/:charId/voice", async (req, res, next) => {
  try {
    ...
    try {
      ...
      const result = await provider.voiceDesign(
        {
          voiceDescription: optimizedVoiceDescription || character.voiceDescription,
          text: testText,
          format: "wav"
        },
        apiKey,
      ...
      const updatedCharacter = await updateAudiobookCharacter(req.params.id, req.params.charId, (target) => {
        target.voiceDataUrl = `data:audio/wav;base64,${result.audioBase64}`;
      ...
      const errorMessage = ttsError instanceof Error ? ttsError.message : "音色生成失败";
      const updatedCharacter = await updateAudiobookCharacter(req.params.id, req.params.charId, (target) => {
        target.voiceStatus = "error";
        target.voiceError = errorMessage;
      });
...
app.post("/api/tts/voicedesign", async (req: Request<unknown, unknown, VoiceDesignPayload>, res, next) => {
  try {
    ...
    const { provider, settings } = await getTTSProvider();
    const result = await provider.voiceDesign(
      { voiceDescription, text, format: "wav" },
      apiKey,
      apiEndpoint
    );

    res.json({
      audioDataUrl: `data:audio/wav;base64,${result.audioBase64}`,
      fileName: `${settings.ttsBackend === "local" ? "local" : "mimo"}-voicedesign-${new Date().toISOString().replace(/[:.]/g, "-")}.wav`,
    ...
    if (error instanceof Error && "status" in error) {
      ...
      const elapsedMs = "elapsedMs" in error ? (error as { elapsedMs?: number }).elapsedMs : undefined;
      return res.status(Number((error as { status?: number }).status) || 502).json({
        error: error.message,
        elapsedMs,
        details
      });
...
app.post("/api/tts/voiceclone", upload.single("voice"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const { provider, settings } = await getTTSProvider();
    const result = await provider.voiceClone(
      { audioBase64, audioMimeType: voiceMime, text, instruction, format: "wav" },
      apiKey,
      apiEndpoint
    );
    ...
    res.json({
      audioDataUrl: `data:audio/wav;base64,${result.audioBase64}`,
      fileName: `${settings.ttsBackend === "local" ? "local" : "mimo"}-voiceclone-${new Date().toISOString().replace(/[:.]/g, "-")}.wav`,
      elapsedMs: result.elapsedMs,
      ttsBackend: settings.ttsBackend
    });
  } catch (error) {
...
let ttsChildProcess: ChildProcess | null = null;
...
function startLocalTTSServer(): void {
  ...
  ttsChildProcess.stdout?.on("data", (data: Buffer) => {
    process.stdout.write(`[TTS] ${data.toString().trim()}\n`);
  });
  ttsChildProcess.stderr?.on("data", (data: Buffer) => {
    process.stderr.write(`[TTS] ${data.toString().trim()}\n`);
  });
  ttsChildProcess.on("close", (code) => {
    console.log(`[TTS] Server exited (code ${code})`);
    ttsChildProcess = null;
  });
  ttsChildProcess.on("error", (err) => {
    console.warn(`[TTS] Failed to start: ${err.message}`);
    ttsChildProcess = null;
  });

  console.log("[TTS] Waiting for model to load (PID %d)...", ttsChildProcess.pid);
}

function stopLocalTTSServer(): void {
  if (!ttsChildProcess) return;
  console.log("[TTS] Stopping local server...");
  ttsChildProcess.kill("SIGTERM");
  ttsChildProcess = null;
}

export function startServer(listenPort = port, host?: string) {
  // 自动启动本地 TTS 服务
  readTTSSettings().then((settings) => {
    if (settings.ttsBackend === "local") {
      startLocalTTSServer();
    }
  }).catch(() => {
...
async function optimizeAudiobookCharacterVoiceDescription(
  ...
  const content = await callLLMWithFallback(payload, apiKey, apiEndpoint);
...
function normalizeApiSettings(value: unknown): ApiSettings {
  ...
  const apiKey = typeof candidate.apiKey === "string" ? String(candidate.apiKey).trim() : "";
  const apiEndpoint = typeof candidate.apiEndpoint === "string" ? String(candidate.apiEndpoint).trim() : "";
  const ttsBackend = candidate.ttsBackend === "mimo-cloud" || candidate.ttsBackend === "local" ? candidate.ttsBackend as ApiSettings["ttsBackend"] : undefined;
  const llmApiKey = typeof candidate.llmApiKey === "string" ? String(candidate.llmApiKey).trim() : undefined;
  const llmEndpoint = typeof candidate.llmEndpoint === "string" ? String(candidate.llmEndpoint).trim() : undefined;
  const llmModel = typeof candidate.llmModel === "string" ? String(candidate.llmModel).trim() : undefined;

  const result: ApiSettings = {};
  if (apiKey) result.apiKey = apiKey;
  if (apiEndpoint) result.apiEndpoint = apiEndpoint;
  if (ttsBackend) result.ttsBackend = ttsBackend;
  if (llmApiKey !== undefined) result.llmApiKey = llmApiKey;
  if (llmEndpoint !== undefined) result.llmEndpoint = llmEndpoint;
  if (llmModel !== undefined) result.llmModel = llmModel;
  return result;
}
...
async function synthesizeAudiobookProduct(
  ...
  // 从 voiceDataUrl 提取 base64 和 mime 类型
  const voiceMatch = character.voiceDataUrl.match(/^data:([^;]+);base64,(.+)$/);
  if (!voiceMatch) {
    throw new Error("角色音色数据格式异常，请重新生成。");
  }

  const { provider } = await getTTSProvider();
  const result = await provider.voiceClone(
    {
      audioBase64: voiceMatch[2],
      audioMimeType: voiceMatch[1],
      text: product.text,
