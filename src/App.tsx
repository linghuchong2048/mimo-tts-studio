...
function StudioApp() {
  ...
  const [ttsBackend, setTtsBackend] = useState<"mimo-cloud" | "local">("mimo-cloud");
  ...
  async function loadApiSettings() {
    try {
      ...
      setTtsBackend(settings.ttsBackend || "mimo-cloud");
      setLlmApiKey(settings.llmApiKey || "");
      setLlmEndpoint(settings.llmEndpoint || "");
      setLlmModel(settings.llmModel || "deepseek-chat");
        ...
        <div className="api-key-modal" onClick={closeApiKeyModal}>
          <div className="api-key-modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="api-key-modal-body">
              {/* ---- 工作模式 ---- */}
              <p className="api-key-modal-hint" style={{ fontWeight: 700, color: "#c5a45d", marginTop: 0 }}>
                工作模式
              </p>
              <div className="tts-backend-toggle">
                <button type="button" className={ttsBackend === "mimo-cloud" ? "tts-toggle-btn active" : "tts-toggle-btn"} onClick={() => setTtsBackend("mimo-cloud")}>
                  云端模式（MiMo）
                </button>
                <button type="button" className={ttsBackend === "local" ? "tts-toggle-btn active" : "tts-toggle-btn"} onClick={() => setTtsBackend("local")}>
                  本地模式（OmniVoice + 自定义LLM）
                </button>
              </div>
              ...
              {ttsBackend === "mimo-cloud" && (
                <div className="api-endpoint-section">
                  <label className="node-field">
                    ...
                    <input type="text" className="api-key-input" value={apiKeyInput} onChange={(e) => setApiKeyInput(e.target.value)} placeholder="sk-..." spellCheck={false} />
                  </label>
                  <label className="node-field">
                    <span>API 地址（可选）</span>
                    <input type="text" className="api-key-input" value={apiEndpointInput} onChange={(e) => setApiEndpointInput(e.target.value)} placeholder="https://api.xiaomimimo.com/v1/chat/completions" spellCheck={false} />
                  </label>
                  <p className="api-endpoint-hint">
                    <a href="https://platform.xiaomimimo.com/" target="_blank" rel="noopener noreferrer">获取 Key</a>
                    {" · "}token 套餐：token-plan-cn.xiaomimimo.com
                  </p>
                </div>
              )}
              ...
              {ttsBackend === "local" && (
                <div className="api-endpoint-section">
                  <p className="api-key-modal-hint" style={{ marginBottom: 8 }}>
                    TTS：OmniVoice @ localhost:8000（启动时自动加载）
                  </p>
                  <label className="node-field">
                    <span>LLM API Key</span>
                    <input type="text" className="api-key-input" value={llmApiKey} onChange={(e) => setLlmApiKey(e.target.value)} placeholder="sk-..." spellCheck={false} />
                  </label>
                  <label className="node-field">
                    <span>LLM 端点</span>
                    <input type="text" className="api-key-input" value={llmEndpoint} onChange={(e) => setLlmEndpoint(e.target.value)} placeholder="https://api.deepseek.com/v1" spellCheck={false} />
                  </label>
                  <label className="node-field">
                    <span>LLM 模型名</span>
                    <input type="text" className="api-key-input" value={llmModel} onChange={(e) => setLlmModel(e.target.value)} placeholder="deepseek-chat" spellCheck={false} />
                  </label>
                </div>
              )}
            </div>
...
function AudiobookConsole({
  ...
  async function mergeAndDownload() {
    const readyProducts = workspace.products.filter((p) => p.status === "ready" && p.audioDataUrl);
    if (readyProducts.length === 0) return;

    setIsGenerating(true);
    try {
      const gapSeconds = 0.4; // 段间停顿
      const audioCtx = new AudioContext({ sampleRate: 24000 });
      const buffers: AudioBuffer[] = [];

      for (let i = 0; i < readyProducts.length; i++) {
        const url = readyProducts[i].audioDataUrl!;
      ...
      const gapFrames = Math.floor(sampleRate * gapSeconds);
      const totalFrames = buffers.reduce((sum, b) => sum + b.length, 0) + gapFrames * (buffers.length - 1);
      ...
      for (let i = 0; i < buffers.length; i++) {
        ...
        offset += buffers[i].length + (i < buffers.length - 1 ? gapFrames : 0);
      }
      ...
      a.href = url;
      a.download = `${sanitizeFileName(workspace.name)}-${formatDateForFile(new Date())}.wav`;
      a.click();
      URL.revokeObjectURL(url);
    } catch (err) {
      console.error("[audiobook:merge] failed", err);
    } finally {
      setIsGenerating(false);
    }
  }
          ...
          <div className="audiobook-section" ref={productSectionRef}>
            <div className="section-header">
              ...
              <div className="product-toolbar">
                {allProductsReady && (
                  <>
                    <button className="run-button" type="button" onClick={() => void mergeAndDownload()}>
                      <FileAudio size={14} />
                      合并下载
                    </button>
